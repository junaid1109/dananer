                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="button--group">
                                            <a href="{{url('admin/language/edit', $item->id)}}" class="btn btn-sm btn-outline--success">
                                                <i class="la la-language"></i> @lang('Translate')
                                            </a>
                                            <a href="javascript:void(0)" class="btn btn-sm btn-outline--primary ms-1 editBtn" data-url="{{ url('admin/language/update', $item->id)}}" data-lang="{{ json_encode($item->only('name', 'text_align', 'is_default','image')) }}" data-image="{{ getImage(getFilePath('language').'/'.$item->image,getFileSize('language')) }}">
                                                <i class="la la-pen"></i> @lang('Edit')
                                            </a>
                                            @if($item->id != 1)
                                                <button class="btn btn-sm btn-outline--danger confirmationBtn" data-question="@lang('Are you sure to remove this language from this system?')" data-action="{{ url('admin/language/delete', $item->id) }}">
    ...
    <div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="form-horizontal" method="post" action="{{ url('admin/language/store')}}" enctype="multipart/form-data">
                    @csrf
    ...
    <script>
        (function($){
            ...
            $('.keyBtn').on('click',function (e) {
                e.preventDefault();
                $.get("{{ url('admin/language/get-keys') }}", {},function (data) {
                    $('.langKeys').text(data);
                });
            });
