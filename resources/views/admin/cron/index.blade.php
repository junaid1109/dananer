                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu p-0">
                                                    ...
                                                    @if ($cron->is_running)
                                                        <a href="{{ url('admin/cron/schedule/pause', $cron->id) }}" class="dropdown-item"><i class="las la-pause"></i> @lang('Pause')</a>
                                                    @else
                                                        <a href="{{ url('admin/cron/schedule/pause', $cron->id) }}" class="dropdown-item"><i class="las la-play"></i> @lang('Play')</a>
                                                    @endif
                                                    ...
                                                    <a href="{{ url('admin/cron/schedule/logs', $cron->id) }}" class="dropdown-item"><i class="las la-history"></i> @lang('Logs')</a>
                                                    @if (!$cron->is_default)
                                                        <a href="javascript:void(0)" data-action="{{ url('admin/cron/delete', $cron->id) }}" data-question="@lang('Are you sure to delete this cron?')" class="dropdown-item confirmationBtn"><i class="las la-trash"></i> @lang('Delete')</a>
                                                    @endif
                                                </div>
                                            </div>
                                        </td>
    ...
    <div class="modal fade" id="addCron" tabindex="-1" role="dialog" a aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="form-horizontal disableSubmission resetForm" method="post" action="{{ url('admin/cron/store') }}">
                    @csrf
    ...
    <div class="modal fade" id="updateCron" tabindex="-1" role="dialog" a aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="form-horizontal resetForm" method="post" action="{{ url('admin/cron/update') }}">
                    @csrf
...
@push('breadcrumb-plugins')
    ...
    <a href="{{ url('admin/cron/schedule') }}" class="btn btn-outline--info"><i class="las la-clock"></i>@lang('Cron Schedule')</a>
@endpush
