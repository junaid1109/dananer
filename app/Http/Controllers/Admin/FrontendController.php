...
class FrontendController extends Controller
{
    ...
    public function importContent(Request $request, $key)
    {
        ...
        foreach ($temPaths as $temp) {
            $arr         = explode('/', $temp);
            $tempname    = end($arr);
            if (activeTemplateName() != $tempname) $templates[] = $tempname;
        }
        ...
        if ($dataContent) {
            ...
            if (@$toTempJson['content']) {
                ...
                $toFrontendContent->slug        = $dataContent->slug;
                $toFrontendContent->seo_content = $dataContent->seo_content;
        ...
        if (@$toTempJson['element']) {
            ...
            foreach ($dataElement as $dataEl) {
                ...
                $toFrontendElement->slug        = $dataEl->slug;
                $toFrontendElement->seo_content = $dataEl->seo_content;
